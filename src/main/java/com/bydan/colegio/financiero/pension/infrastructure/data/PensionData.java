...
public class PensionData implements PensionDataI {
	...
	@Autowired
	private PensionCrudRepositoryI pension_crudrepositoryi1;
	...
	public ArrayList<Pension> getTodos(Pagination pagination1) {  
        try	{			
			...
			this.pensionModels = this.pension_crudrepositoryi1.findAll();				
	...
	public Pension getBuscarUno(Long id) {  
        try	{			
            ...
            this.pensionModel1 = this.pension_crudrepositoryi1.findById(id);
    ...
    public Pension nuevo(Pension pension1) {
        try	{			           
			...
			pensionModel2 = this.pension_crudrepositoryi1.save(pensionModel2);
	...
	public void eliminar(Long id) {  
        try	{			          
			this.pension_crudrepositoryi1.deleteById(id);
	...
	public ArrayList<Pension> nuevos(ArrayList<Pension> pensions) {
        try	{		
			...
			this.pensionModels = this.pension_crudrepositoryi1.saveAll(pensionModels2);
	...
	public void eliminars(ArrayList<Long> ids) {  
        try	{			          
			this.pension_crudrepositoryi1.deleteAllById(ids);
	...
	public void actualizars(ArrayList<Pension> updates_pensions,ArrayList<String> updates_columnas) {  
        try	{			           
			...
			//this.pensionModels = this.pension_crudrepositoryi1.update('id',updates_pensions);
	...
	public PensionCrudRepositoryI getPensionCrudRepositoryI1() {
		return this.pension_crudrepositoryi1;	
	}
	
	public void setPensionCrudRepositoryI1(PensionCrudRepositoryI pension_crudrepositoryi1) {
		this.pension_crudrepositoryi1 = pension_crudrepositoryi1;
	}
}
